...
if(isMultipart)
{
   ...
   Iterator itr=items_list.iterator();
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           if(field_name.equals("fl_productimg"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Assets\\Product Image\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="fl_productimg_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String str1="insert into tbl_product(category_id,product_name,product_img)"
              +"values('"+value[0]+"','"+value[1]+"','"+fn+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../Admin/ProductManagement.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
    else{
        %>
        <script>alert("Failed upload");</script>
        <%
    }
}
