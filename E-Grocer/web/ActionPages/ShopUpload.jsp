...
if(isMultipart)
{
   ...
   Iterator itr=items_list.iterator();
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           if(field_name.equals("fl_shopimg"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Assets\\Shop\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="shopimg_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
           ...
           if(field_name.equals("fl_shopproof"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            ...
            fn1="shopproof_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
            //creating a file object
           ...
           if(field_name.equals("fl_idproof"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            ...
            fn2="ownerproof_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
            //creating a file object
    ...
    String sel1 = "select * from tbl_user where email = '" + value[8] + "'";
                String sel2 = "select * from tbl_user where contact = '" + value[9] + "'";
                String sel3 = "select * from tbl_shop where shop_email = '" + value[8] + "'";
                String sel4 = "select * from tbl_shop where shop_contact = '" + value[9] + "'";
                String sel5 = "select * from tbl_deliveryagent where da_email = '" + value[8] + "'";
                String sel6 = "select * from tbl_deliveryagent where da_contact = '" + value[9] + "'";
        ...
        <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../Guest/NewShop.jsp'},40);
        </script>
        <%
                }
   ...
   String str1="insert into tbl_shop(shop_name,shop_img,shop_licsno,shop_gstno,shop_proof,shop_owner,idproof,shop_address,place_id,shop_pincode,shop_email,shop_contact,shop_pwd)"
              +"values('"+value[0]+"','"+fn+"','"+value[1]+"','"+value[2]+"','"+fn1+"','"+value[3]+"','"+fn2+"','"+value[4]+"','"+value[6]+"','"+value[7]+"','"+value[8]+"','"+value[9]+"','"+value[10]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../Guest/Login.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }else{
